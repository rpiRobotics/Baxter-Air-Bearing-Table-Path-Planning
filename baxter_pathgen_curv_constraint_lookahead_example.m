...
for k = 1:length(lambda)
    ...
    if k<length(lambda)-curv_win-1
        if lambda(k+curv_win) <=0.5
            vt_next = (1/0.5)*(P0T1 - P0T0);
        else
            vt_next = (1/0.5)*(P0Td - P0T1);
        end
    else
        vt_next = (1/0.5)*(P0Td - P0T1);
    end
    ...
    H = getqp_H_ma(14, 2, [J,zeros(6,7);zeros(6,7),J], ...
        [vr;vt;der_dlambda*k_hat;vt_next], epsilon_r, epsilon_p, 0.0001);
    f = getqp_f([qprev;qprev], epsilon_r, epsilon_p );
    ...
    A = -[dhdq(:,1:7),zeros(14,7),dhdq(:,8:9)];
    b = -sigma;
    
    if (q_curve_limit_on)&&(k>1)
        A = [A;eye(7),zeros(7,9);-eye(7),zeros(7,9);-eye(7),eye(7),zeros(7,2);eye(7),-eye(7),zeros(7,2)];
        b = [b;(1/N)*q_primeprime_max + q_prime_prev;...
            -((1/N)*q_primeprime_min + q_prime_prev);(curv_win/N)*q_primeprime_max;...
            -(curv_win/N)*q_primeprime_min];
    end
    ...
    [q_prime_temp,~,exitflag(k)] = quadprog(H,f,A,b,Aeq,beq,lb,ub,[]...
        ,options);
    debug(:,k) = q_prime_temp(1:7)-q_prime_temp(8:14);
